...
- Category, city, status, and activity comparisons now normalize invisible Unicode characters and whitespace from Google Sheets.
- Employee-specific Telegram menu uses a valid `Старт` description and menu API errors no longer block `/start` responses.
