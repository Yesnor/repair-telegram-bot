...
const { createRequest, findRequestById, saveRequest, STATUS } = require('./requests');
const { getEmployeeByTelegramId, getActiveEmployeesByCategory } = require('./employees');
...
const requestWizard = new Scenes.WizardScene(
  ...
  async (ctx) => {
    await ctx.reply(
      ...
      Markup.inlineKeyboard(CATEGORIES.map((c) => Markup.button.callback(c, `cat:${c}`)))
    );
  ...
  async (ctx) => {
    if (!ctx.callbackQuery || !ctx.callbackQuery.data?.startsWith('cat:')) {
      await ctx.reply('Пожалуйста, выберите категорию, нажав на одну из кнопок выше.');
      return;
    }
  ...
  async (ctx) => {
    ...
    if (!phone) {
      await ctx.reply('Пожалуйста, укажите телефон текстом или кнопкой "Отправить мой номер".');
      return;
    }

    const data = ctx.wizard.state.data;
    const clientName = [ctx.from.first_name, ctx.from.last_name].filter(Boolean).join(' ');
...
async function notifyEmployees(ctx, requestData, excludeEmployeeIds = []) {
  ...
  const employees = allEmployees.filter(
    (e) => !excludeEmployeeIds.some((id) => String(id) === String(e.telegramId))
  );
  ...
  for (const emp of employees) {
    try {
      const msg = await ctx.telegram.sendMessage(
        ...
        Markup.inlineKeyboard([Markup.button.callback('Взять в работу', `take:${requestData.id}`)])
      );
      notified.push({ chatId: emp.telegramId, messageId: msg.message_id });
    } catch (err) {
      console.error(`Не удалось отправить уведомление сотруднику ${emp.telegramId}:`, err.message);
    }
  }
...
async function markOtherNotifications(ctx, requestData, exceptEmployeeId, note) {
  let notified = [];
  try {
  ...
  for (const item of notified) {
    ...
    try {
      await ctx.telegram.editMessageText(item.chatId, item.messageId, undefined, note);
    } catch {
...
bot.start(async (ctx) => {
  ...
  await ctx.reply(
    ...
    Markup.inlineKeyboard([Markup.button.callback('📝 Оставить заявку', 'new_request')])
  );
});
...
bot.action(/^take:(.+)$/, async (ctx) => {
  ...
  const employee = await getEmployeeByTelegramId(ctx.from.id);
  if (!employee) {
    await ctx.answerCbQuery('Вы не зарегистрированы как сотрудник.', { show_alert: true });
    return;
  }
  ...
  if (found.data.status !== STATUS.NEW) {
    await ctx.answerCbQuery('Заявка уже взята другим сотрудником.', { show_alert: true });
    try {
  ...
  await ctx.editMessageText(
    ...
    Markup.inlineKeyboard([
      ...
      [Markup.button.callback('↩️ Отказаться от заявки', `decline:${requestId}`)],
    ])
  );

  await notifyClient(ctx, found.data, `Ваша заявка №${requestId} взята в работу мастером ${employee.name}.`);
  await markOtherNotifications(ctx, found.data, employee.telegramId, 'Заявка уже взята другим сотрудником.');
});
...
bot.action(/^depart:(.+)$/, async (ctx) => {
  ...
  if (!found || !employee || String(found.data.employeeId) !== String(employee.telegramId)) {
  ...
  await ctx.editMessageText(
    ...
    Markup.inlineKeyboard([[Markup.button.callback('☑️ Закрыть заявку', `close:${requestId}`)]])
  );

  await notifyClient(ctx, found.data, `Мастер выехал на адрес по заявке №${requestId}.`);
});
...
bot.action(/^close:(.+)$/, async (ctx) => {
  ...
  await notifyClient(ctx, found.data, `Заявка №${requestId} выполнена и закрыта. Спасибо, что обратились к нам!`);
});
